...
public class DataCleanUtil {
    ...
    private static DataCleanUtil INSTACE = null;
    private List<String> paths = new ArrayList<>();

    public static DataCleanUtil getInstance() {
        if (INSTACE == null) INSTACE = new DataCleanUtil();
        return INSTACE;
    }
    ...
    public String getTotalCacheSize(Context context) throws Exception {
        ...
        if (paths != null) {
            for (String path : paths) {
                ...
                cacheSize += getFolderSize(picFile);
            }
    ...
    private void deleteFilesByDirectory(File directory) {
        if (directory != null && directory.exists() && directory.isDirectory()) {
            for (File item : directory.listFiles()) {
                if (item.isDirectory()) {
                    deleteFilesByDirectory(item.getAbsoluteFile());
                } else {
                    item.delete();
                }
            }
