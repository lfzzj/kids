...
public class ToastUtil {
   ...
   /* public static void showShort(Context context, CharSequence message) {
     ...
     * 短时显示Toast
     */
    public static void showShortError(String message) {
        if (isShow) {
            if (mToast == null) {
                mToast = Toast.makeText(BaseApplication.getInstance(), StringUtil.enToCn(message), Toast.LENGTH_SHORT);
            } else {
                mToast.setText(StringUtil.enToCn(message));
            }
            mToast.setGravity(Gravity.CENTER, 0, 0);
            mToast.show();
        }
