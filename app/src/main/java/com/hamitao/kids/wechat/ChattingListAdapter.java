...
public class ChattingListAdapter extends BaseAdapter {
    ...
    public ChattingListAdapter(Activity context, Conversation conv, ContentLongClickListener longClickListener, String channelid) {
    ...
    public ChattingListAdapter(Context context, Conversation conv, ContentLongClickListener longClickListener, int msgId, String channelid) {
        this.mContext = context;
        ...
        if (holder.headIcon != null) {
            if (userInfo != null && !TextUtils.isEmpty(userInfo.getAvatar())) {
                userInfo.getAvatarBitmap(new GetAvatarBitmapCallback() {
                    ...
                    public void gotResult(int status, String desc, Bitmap bitmap) {
                        if (status == 0) {
                            holder.headIcon.setImageBitmap(ImageUtils.createCircleImage(bitmap));
                        } else {
                            holder.headIcon.setImageResource(R.drawable.icon_head_default);
                        }
                    }
                });
            } else {
...
//                    ToastUtil.showShort("点击头像跳转到个人信息界面");
...
//                            ToastUtil.showShort("群聊未读消息数点击事件");
...
//                                            BaseApplication.alreadyRead.clear();
//                                            BaseApplication.unRead.clear();
//                                            List<UserInfo> alreadyRead = receipt.getReceiptList();
//                                            List<UserInfo> unRead = receipt.getUnreceiptList();
//                                            BaseApplication.alreadyRead.addAll(alreadyRead);
//                                            BaseApplication.unRead.addAll(unRead);
...
//        }
    ...
    public void showResendDialog(final ViewHolder holder, final Message msg) {
        ...
        ((BaseActivity) mActivity).showConfirmDialog(mActivity, "重发消息吗？", new OnViewClickListener() {
            @Override
